...
class Clause:
    ...
    _data: dict[type[AtomicFormula], set[AtomicFormula]] = field(
        default_factory=lambda: {Eq: set(), Ne: set(), Ge: set(), Gt: set(), Le: set(), Lt: set()})
    ...
    @classmethod
    def from_disjunction(cls, f: Or) -> Self:
        clause = cls()
        for arg in f.args:
    ...
    def is_equational(self) -> bool:
        return not any(self[rel] for rel in (Ne, Le, Ge, Lt, Gt))

    def to_formula(self) -> Formula:
        return Or(*set().union(*self._data.values()))


def formula_to_clauses(f: Formula) -> tuple[list[Clause], bool]:
    ...
    for arg in f.args:
        if isinstance(arg, Or):
            ...
            assert isinstance(arg, AtomicFormula)
            clauses.append(Clause.from_atom(arg))
    return clauses, negated

def gsimplify(f: Formula, assume: Iterable[AtomicFormula] = []) -> Formula:
    f = simplify(f, assume=assume)
    if isinstance(f, (AtomicFormula, _T, _F)):
        return f
    clauses, negate = formula_to_clauses(f)
    clauses = gsimplify_clauses(clauses, assume=assume)
...
def gsimplify_clauses(clauses: list[Clause], assume: Iterable[AtomicFormula] = []) -> list[Clause]:
    logging.info(f'{len(clauses)} clauses to process')
    assumed_eq = {atom.lhs for atom in assume if isinstance(atom, Eq)}

    # Remove equational clauses entailed by the assumed equations. Add all
    # other equational clauses to the assumed equations.
    assumed_eq_gb = Term.gbasis(list(assumed_eq))
    for clause in clauses.copy():
        if clause.is_equational():
            product = clause.as_product(Eq)
            if in_radical(product, assumed_eq_gb):
            ...
            if isinstance(simplified_equation, _F):
                return [Clause()]
            if product not in assumed_eq:
                assumed_eq.add(product)

    # Now simplify the other clauses.
    for clause in clauses.copy():
        if not clause.is_equational():
            this_assumed_eq = assumed_eq | {atom.lhs for atom in clause[Ne]}
            this_assumed_eq_gb = Term.gbasis(list(this_assumed_eq))

            # Process the product of all equations together with factors from weak inequalities.
            product = clause.as_product(Eq) * clause.as_product(Le) * clause.as_product(Ge)
            if in_radical(product, this_assumed_eq_gb):
                clauses.remove(clause)
                continue
            ...
            if isinstance(simplified_equation, _F):
                ...
                # strict inequalities. The following deletion affects both clause and clause.copy().
                clause[Eq] = set()

            # Equations are fine now. Process all inequalities. We radical membership test for the
            # strict one but not for the weak ones anymore.
            for strict_rel in (Gt, Lt):
                for atom in clause[strict_rel]:
                    if in_radical(atom.lhs, this_assumed_eq_gb):
                        clause[strict_rel] -= {atom}

            try:
                for rel in (Ge, Gt, Le, Lt):
...
def in_radical(f: Term, G: list[Term]) -> bool:
    y = VV.fresh()
