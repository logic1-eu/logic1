...
Internally, simplification now works with monic polynomials over $\mathbb{Q}$.

### Rational coefficients in terms

`RCF.Term` now uses rational coefficients instead of integer coefficients. The implementation expects and uses `mpq` values for coefficient input and output.

### Reduced Sage dependency

The `RCF` simplification infrastructure no longer depends on Sage. In particular, Sage `Rational` values have been replaced by `mpq`-based arithmetic in simplification routines.
